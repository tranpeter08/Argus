...
describe("Employees API", function(){
  ...
  describe('POST endpoint', function () {
    ...
    describe('requests with a valid token', function () {
      ...
      it('should reject a request with missing fields', function () {
        const data = {
          employeeName: {},
          certifications: [],
          notes: ''
        ...
        return chai.request(app)
          ...
          .then(res => {
            ...
            expect(res.body.message).to.equal('Missing "equipment" in request body')
          });
      });

      it('should reject a request with missing name fields', function () {
        const data = {
          employeeName: {
            firstName: 'pop'
          },
        ...
        return chai.request(app)
          ...
          .then(res => {
            expect(res).to.have.status(400);
            expect(res.body.message).to.equal('Missing "lastName" in employeeName');
          });
      });
  ...
  describe("PUT endpoint", function () {
    ...
    describe("requests with a valid token", function () {
      ...
      it('should reject requests if body id and param id do not match', function () {
        const paramId = '1234';
        const bodyId = 'abcd';

        return chai
          .request(app)
          .put(`/employees/${paramId}`)
          .set('Authorization', `Bearer ${token}`)
          .send({bodyId})
          .then(res => {
            expect(res).to.have.status(400);
            expect(res.body.message).to.equal('response body and parameter ID do not match')
          })
      });
